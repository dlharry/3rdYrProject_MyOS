...
#define MAIL_TAGS     0x8 // Mailbox Channel 8: Tags (ARM to VC) ***what we use***
...
static u32 mail_box_read(u8 channel){
    while(1){
        ...
        // loop infinite until we find that channel.
...
bool mailbox_process(mailbox_tag *tag, u32 tag_size){
    int buffer_size = tag_size + 8 + 4; // command struct size
