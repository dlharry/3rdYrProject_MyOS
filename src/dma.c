...
static u16 allocate_channel(u32 channel){ // not really allocating memory
    if (!(channel & ~0x0F)) { // check if there is any bits passed in first 15
        if (channel_map & (1 << channel)) { // if there is a bit set for specific channel, unset it
    ...
    for (; i >= 0; i--) {
        if (channel_map & (1 << i)) { // find the one that has the bit set
            channel_map &= ~(1 << i); // unset the channel and return for our use
            return i;
        }
    }
...
dma_channel *dma_open_channel(u32 channel) {
    ...
    dma->block = (dma_control_block *)((LOW_MEMORY + 31) & ~31); // align the pointer
    dma->block->res[0] = 0;
    dma->block->res[1] = 0;

    REGS_DMA_ENABLE |= (1 << dma->channel); //enable
    timer_sleep(3); // leave it some time to set up
    REGS_DMA(dma->channel)->control |= CS_RESET; // set the reset flag
...
bool dma_wait(dma_channel *channel) {
    ...
    channel->status = REGS_DMA(channel->channel)->control & CS_ERROR ? false : true; // false if failed
