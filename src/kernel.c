...
void kernel_main(){
    ...
    while(1){
        ...
        if(result != -1){
            // win
            drawString((WIDTH/2)-(END_WIDTH/2), (HEIGHT/2)-(END_HEIGHT/2)+END_HEIGHT, my_sprintf("You scored %d!", result), 0x02, END_ZOOM);
        } 
        drawString((WIDTH/2)-(END_WIDTH/2), (HEIGHT/2)-(END_HEIGHT/2)+2*END_HEIGHT, "Replay?", 0x02, END_ZOOM);
    }
